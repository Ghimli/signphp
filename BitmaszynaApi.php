...
/**
 *
 * @author Jan Moritz Lindemann + Damian 'Ghimli' Obernikowicz
 */
		...
		} else if ($derEvenOrOddCode == '03') // odd
{
	...
	public function getSignatureHashPoints($hash, $nonce = null) {
		...
		$S = gmp_strval ( gmp_mod ( gmp_mul ( gmp_invert ( gmp_init ( $nonce, 16 ), $n ), gmp_add ( gmp_init ( $hash, 16 ), gmp_mul ( gmp_init ( $k, 16 ), gmp_init ( $R, 16 ) ) ) ), $n ), 16 );
	...
	/**
	 * *
	 * Satoshi client's modified message signature implementation.
	 *
	 * @param
	 *        	$message
	 * @param
	 ...
	 * @throws \Exception
	 */
	public function signature($message, $secret) {
		$this->setPrivateKey ( bin2hex ( base64_decode ( $secret ) ) );
		$hash = $this->hash256 ( "Bitmaszyna.pl API:\n" . $message );
		$points = $this->getSignatureHashPoints ( $hash, null );
		$R = $points ['R'];
		$S = $points ['S'];
		while ( strlen ( $R ) < 64 )
			$R = '0' . $R;
		...
		for($i = 0; $i < 4; $i ++) {
			...
			if ($this->getDerPubKeyWithPubKeyPoints ( $pubKeyPts, false ) == $recoveredPubKey) {
				$finalFlag = $flag;
			}
		}
		if (0 == $finalFlag) {
		...
		return (base64_encode ( hex2bin ( dechex ( $finalFlag ) . $R . $S ) ));
	}
		...
		if ($flag >= 31) // if address is compressed
{
		...
		// step 1.1
		$x = null;
		$x = gmp_add ( gmp_init ( $R, 16 ), gmp_mul ( $this->n, gmp_div_q ( // check if j is equal to 0 or to 1.
gmp_init ( $recid, 10 ), gmp_init ( 2, 10 ) ) ) );
		...
		if (1 == $flag % 2) // check if y is even.
{
	...
	public function checkSignaturePoints($pubKey, $R, $S, $hash) {
		...
		// S^-1* hash
		$exp1 = gmp_strval ( gmp_mul ( gmp_invert ( gmp_init ( $S, 16 ), $this->n ), gmp_init ( $hash, 16 ) ), 16 );
		
		// S^-1* hash * G
		$exp1Pt = $this->mulPoint ( $exp1, $G );
		
		// S^-1 * R
		$exp2 = gmp_strval ( gmp_mul ( gmp_invert ( gmp_init ( $S, 16 ), $this->n ), gmp_init ( $R, 16 ) ), 16 );
		// S^-1 * R * Qa
